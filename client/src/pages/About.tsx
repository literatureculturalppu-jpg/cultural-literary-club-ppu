      ...
      <section className="bg-gradient-to-b from-accent/10 to-background py-12 md:py-16">
        <div className="container">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl">
            تعرف على تاريخ النادي وأهدافه وهيئته الإدارية
          </p>
        </div>
      </section>
          ...
          {teamMembersLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {teamMembers.map((member) => (
                <Card key={member.id} className="overflow-hidden hover:shadow-lg transition-shadow h-full text-center p-6">
                  {member.imageUrl ? (
                    <img
                      src={member.imageUrl}
                      alt={member.name}
                      className="w-28 h-28 rounded-full object-cover mx-auto mb-4 border-4 border-accent/10"
                      loading="lazy"
                      decoding="async"
                    />
                  ) : (
                    <div className="w-28 h-28 rounded-full bg-gradient-to-br from-accent/20 to-accent/5 flex items-center justify-center mx-auto mb-4">
                      <Users className="w-10 h-10 text-accent" />
                    </div>
                  )}
                  <h3 className="text-xl font-bold text-foreground">{member.name}</h3>
                  <p className="text-accent text-sm font-medium mt-1">{member.position}</p>
                  {member.bio && (
                  ...
                  {(member.email || member.phone) && (
                    <div className="flex items-center justify-center gap-4 mt-4 text-xs text-muted-foreground">
                      {member.email && (
                        <span className="flex items-center gap-1" dir="ltr"><Mail className="w-3.5 h-3.5" />{member.email}</span>
                      )}
                      {member.phone && (
                        <span className="flex items-center gap-1" dir="ltr"><Phone className="w-3.5 h-3.5" />{member.phone}</span>
                      )}
                    </div>
                  )}
                </Card>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground text-lg">
                سيتم إضافة أعضاء الهيئة الإدارية قريباً
              </p>
            </div>
          )}
        </div>
