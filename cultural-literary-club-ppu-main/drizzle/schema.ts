...
export const meetingParticipantStatusEnum = pgEnum("meeting_participant_status", [
  "waiting", // in the waiting room, awaiting admin/tech_admin approval
  "admitted", // approved and currently (or previously) in the room
  "rejected", // waiting-room request explicitly denied
...
export const teamMembers = pgTable("teamMembers", {
  ...
  // Marks the original founding board (first president, first vice
  // president, first tech admin, etc.). Whoever succeeds them later is
  // added as a normal (non-founder) row — this flag is never set from the
  // regular "add team member" flow, only via the founders seed script.
...
// NOTE: the former standalone "work teams" feature (workTeams /
// workTeamMembers tables) shown on the About page has been retired in
// favor of the existing `teamMembers` table above ("الهيئة الإدارية" —
// the administrative board). See drizzle/0004_drop_work_teams.sql.
...
export const workLogs = pgTable("workLogs", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  /**
   * A work log entry is never deleted immediately. Only the Technical
   * Manager ("المدير التقني") can request deletion, which sets this to
   * now + 48h; the daily cron (runScheduledWorkLogsCleanup, see
   * /api/cron/worklogs-cleanup) actually removes the row once this
   * timestamp has passed. Null = not scheduled for deletion.
   */
  scheduledDeleteAt: timestamp("scheduledDeleteAt"),
});
 ...
 *
 * This table stores ONLY transient metadata about a meeting room: who
 * created it, when it is/was live, and its current moderation settings.
 * It intentionally never stores video, audio, recordings, or chat text —
 * actual media flows through a self-hosted LiveKit server and signaling /
 * presence / ephemeral chat flow through Supabase Realtime, neither of
 * which touch this database. Rows for `ended`/`cancelled` meetings are
 * deleted by the daily cron cleanup (see server/services/meetingsCleanup.ts)
 * after a short grace period, same pattern as the books-cleanup cron.
 */
export const meetings = pgTable("meetings", {
  id: serial("id").primaryKey(),
  /** LiveKit room name — unique, never reused. */
  roomName: varchar("roomName", { length: 64 }).notNull().unique(),
  title: varchar("title", { length: 255 }),
  createdBy: integer("createdBy").notNull(), // Reference to users.id
  status: meetingStatusEnum("status").default("scheduled").notNull(),
  /** Null for an instant meeting (goes live immediately on creation). */
  scheduledStartAt: timestamp("scheduledStartAt"),
  startedAt: timestamp("startedAt"),
  endedAt: timestamp("endedAt"),
  /** Invite token used to build the shareable join link. */
  inviteToken: varchar("inviteToken", { length: 64 }).notNull().unique(),
  inviteRevoked: boolean("inviteRevoked").default(false).notNull(),
  /** "قفل الاجتماع" — blocks any new joins (even with a valid invite link). */
  locked: boolean("locked").default(false).notNull(),
  /** Default-on per the spec: screen share is blocked for non-moderators unless explicitly allowed. */
  screenShareBlocked: boolean("screenShareBlocked").default(true).notNull(),
  micBlocked: boolean("micBlocked").default(false).notNull(),
  cameraBlocked: boolean("cameraBlocked").default(false).notNull(),
  chatBlocked: boolean("chatBlocked").default(false).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
/**
 * One row per user per meeting: tracks waiting-room requests, admission,
 * kicks, and voluntary departures. Also doubles as the "current attendee
 * count" source (count of rows with status = "admitted" and leftAt null).
 */
export const meetingParticipants = pgTable("meetingParticipants", {
  id: serial("id").primaryKey(),
  meetingId: integer("meetingId").notNull(), // Reference to meetings.id
  userId: integer("userId").notNull(), // Reference to users.id
  status: meetingParticipantStatusEnum("status").default("waiting").notNull(),
  requestedAt: timestamp("requestedAt").defaultNow().notNull(),
  respondedAt: timestamp("respondedAt"),
  respondedBy: integer("respondedBy"), // Reference to users.id (who admitted/rejected/kicked)
...
/**
 * Permanent, club-wide meeting ban — separate from a "kick", which only
 * removes someone from the current meeting. A banned user cannot even
 * enter the waiting room of any future meeting.
 */
export const meetingBans = pgTable("meetingBans", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull().unique(), // Reference to users.id
  bannedBy: integer("bannedBy").notNull(), // Reference to users.id
  reason: text("reason"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type MeetingBan = typeof meetingBans.$inferSelect;
export type InsertMeetingBan = typeof meetingBans.$inferInsert;

/**
 * "صلاحية كسر القيود" — an explicit, revocable grant from the club
 * founder (see isClubFounder() in server/services/meetingsAuth.ts) that
 * lets a specific admin/tech_admin bypass meeting-level restrictions
 * (mic/camera/screen-share/chat blocks) even when those restrictions are
 * active. NOT granted automatically to every admin/tech_admin — the spec
 * is explicit that this must be a separate, opt-in toggle.
 */
export const meetingOverridePermissions = pgTable("meetingOverridePermissions", {
  id: serial("id").primaryKey(),
  userId: integer("userId").notNull().unique(), // Reference to users.id
  grantedBy: integer("grantedBy").notNull(), // Reference to users.id (the founder)
  active: boolean("active").default(true).notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});

export type MeetingOverridePermission = typeof meetingOverridePermissions.$inferSelect;
export type InsertMeetingOverridePermission = typeof meetingOverridePermissions.$inferInsert;

/**
 * "طلب معلومات الانضمام كضيف" — collected from anyone who tries to join an
 * electronic meeting WITHOUT a club account. Same required fields as
 * `guestActivityRegistrations` (guest activity sign-up), since a guest
 * joining a meeting is the same kind of outside visitor as a guest
 * registering for an activity.
 *
 * `meetingTitle`/`meetingDate` are snapshotted at submission time (rather
 * than joined against `meetings` on read) because meeting rows are purged
 * by the meetings-cleanup cron shortly after the meeting ends — the guest
 * request list in the dashboard must still be indexable by meeting
 * name/date long after the meeting itself is gone.
 */
export const meetingGuestJoinRequests = pgTable("meetingGuestJoinRequests", {
  id: serial("id").primaryKey(),
  meetingId: integer("meetingId").notNull(), // Reference to meetings.id (may no longer exist)
  meetingTitle: varchar("meetingTitle", { length: 255 }), // Snapshot of meetings.title at submission time
  meetingDate: timestamp("meetingDate"), // Snapshot of the meeting's scheduled/started date
  fullName: varchar("fullName", { length: 255 }).notNull(),
  universityEmail: varchar("universityEmail", { length: 320 }).notNull(),
  universityId: varchar("universityId", { length: 50 }).notNull(),
  college: varchar("college", { length: 255 }),
