/**
 * Resolves the public base URL used to build absolute links inside emails
 * and notifications.
 *
 * Order of precedence:
 *  1. `APP_BASE_URL` — explicit override, always wins when set (works for
 *     any host: Vercel, a custom domain, Netlify, self-hosted, ...).
 *  2. `VERCEL_PROJECT_PRODUCTION_URL` — the project's stable production
 *     domain, automatically provided by Vercel. Preferred over `VERCEL_URL`
 *     because that one changes per-deployment (includes a random suffix).
 *  3. `VERCEL_URL` — the current deployment's URL, provided by Vercel on
 *     every deployment (production or preview). Used as a fallback so
 *     preview deployments still produce a working (if deployment-specific)
 *     link instead of silently falling back to localhost.
 *  4. `http://localhost:$PORT` — local development only.
 *
 * Without this, a production deployment on Vercel that forgot to set
 * `APP_BASE_URL` would silently send emails containing links like
 * `http://localhost:3000/activities/12`, which are meaningless to anyone
 * outside the machine that sent them.
 */
function resolveAppBaseUrl(): string {
  ...
  if (vercelDeploymentHost) return `https://${vercelDeploymentHost}`;
...
export const ENV = {
  ...
  // See resolveAppBaseUrl() above — falls back to Vercel's own domain env
  // vars before ever falling back to localhost.
  ...
  // Supabase Realtime — signaling, presence, and ephemeral chat only. Point
  // this at the SAME Supabase project already used for Postgres (Realtime
  // is just a feature you enable on that project, not a separate product),
  // unless you have a specific reason to isolate it in its own project.
  supabaseUrl: process.env.SUPABASE_URL ?? "",
  supabaseAnonKey: process.env.SUPABASE_ANON_KEY ?? "",
  // LiveKit — actual audio/video media transport. Works with either:
  //  - LiveKit Cloud (https://cloud.livekit.io) — the easiest option: sign
  //    up, create a project, copy the URL/key/secret below. Free tier
  //    covers small clubs comfortably and needs zero server maintenance.
  //  - A self-hosted LiveKit server (e.g. on Oracle Cloud's free tier) —
  //    more setup (Docker/TURN/systemd) but no usage ceiling. Same env
  //    vars either way; only the values differ.
  livekitUrl: process.env.LIVEKIT_URL ?? "", // wss://your-project.livekit.cloud (or wss://your-domain)
